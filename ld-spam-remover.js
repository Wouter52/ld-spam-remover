...
// @version  1
// @grant    none
// @match        https://ldjam.com/feed/raw
...
const getSpamStatus = function (index, body, name, title) {
    ...
    if (users[name].gameCount === 0) {
        ...
        let links = (body.textContent.match(/<link \/>/g) || []).length;
...
const clean = function () {
    ...
    if (document.getElementsByClassName("page-home-feed-raw").length <= 0) return;

    let elements = document.getElementsByClassName("page-home-feed-raw")[0].getElementsByClassName("content-base content-common content-simple");
    let countRemoved = 0;
